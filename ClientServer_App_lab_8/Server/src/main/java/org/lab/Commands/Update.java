    ...
    public CommandExecute execute(Request o) throws SQLException {
        ...
        try {
            ...
            if (resultSet.next()) {
                ...
                name = (String) updater.compare(data[0].replaceAll("~", " "), resultSet.getString("name"));
                xCoordinates = Integer.parseInt(updater.compare(data[1], resultSet.getInt("coordinateX")).toString());
                yCoordinates = Long.parseLong(updater.compare(data[2], resultSet.getLong("coordinateY")).toString());
                annualTurnover = Double.parseDouble(updater.compare(data[3], resultSet.getDouble("annualTurnover")).toString());
                creationDate = resultSet.getDate("creationDate").toLocalDate();
                fullName = (String) updater.compare(data[4].replaceAll("~", " "), resultSet.getString("fullName"));
                employeesCount = Integer.parseInt(updater.compare(data[5], resultSet.getInt("employeesCount")).toString());
                type = (String) updater.compare(data[6], resultSet.getString("type"));
                street = (String) updater.compare(data[7].replaceAll("~", " "), resultSet.getString("street"));
                xLocation = Long.parseLong(updater.compare(data[8], resultSet.getLong("locationX")).toString());
                yLocation = Float.parseFloat(updater.compare(data[9], resultSet.getFloat("locationY")).toString());
                zLocation = Float.parseFloat(updater.compare(data[10], resultSet.getFloat("locationZ")).toString());
